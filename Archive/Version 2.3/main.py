...
async def display_scripts(interaction, message, scripts, api):
    if not scripts:
    ...
    while True:
        ...
        # --- Navigation Row (Row 0): First, Previous, Page Indicator, Next, Last ---
        if total_items > 1:
            if page > 1:
        ...
        # --- Action Row (Row 1): View, Raw, Download, Copy ---
        if api == "scriptblox":
...
def create_embed(script, page, total_items, api):
    ...
    if api == "scriptblox":
        ...
        truncated_script = truncate_script(script.get("script", "No Script"))
        ...
        set_image(embed, game_image_url)
    elif api == "rscripts":
...
def truncate_script(script_content, max_length=400):
    return script_content if len(script_content) <= max_length else script_content[:max_length - 3] + "..."
...
class APISelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "scriptblox":
            ...
            scripts, _, error = fetch_scripts("scriptblox", self.query, self.mode, 1)
            if error:
                await interaction.followup.send(error)
                return
            await display_scripts(interaction, temp_msg, scripts, api="scriptblox")
            ...
            if error:
                await interaction.followup.send(error)
                return
            await display_scripts(interaction, temp_msg, scripts, api="rscripts")
